...
function AiAgentChat({videoId}: {videoId: string}) {
    ...
    const { messages, input, handleInputChange, handleSubmit, append, status } = useChat({
      ...
      //* Debugging , to test the comms from claude . REMOVE IT ON PRODUCTION */
      onResponse: (response) => {
        // This is called when the API responds
        console.log("Got response:", response);
      },
      onFinish: (message) => {
        // This is called when the streaming is complete
                ...
                {messages.map(m => (
                    <div key={m.id}  className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                      <div  className ={`max-w-[85%] 
                        ${m.role === 'user' ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-800'} rounded-2xl px-4 py-3`}>
                            ...
                            // AI agent messsage
                              <div className='space-y-3'>
                                ...
                                part.type === "text" ? (
                                  ...
                                  <div key={i} 
                                    className='bg-white/50 p-2 rounded-lg text-sm space-y-2 text-gray-800'>
        ...
        <div className='border-t border-gray-100 p-4 bg-white'>
          <div className='space-y-3'>
            <form onSubmit={handleSubmit} className='flex gap-2'>
                <input
                  className='flex-1 px-4 py-2 text-sm border border-gray-200 rounded-full  focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-800'
                  type='text'
                  placeholder={
